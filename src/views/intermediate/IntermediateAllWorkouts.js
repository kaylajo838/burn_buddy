...
export default function IntermediateAllWorkouts() {
    ...
    const handleExpandClick = (index) => {
        setIntermediateWorkout(prevWorkouts =>
    ...
    const fitnessTest = async () => {
        ...
        try {
            ...
            let workoutJson = result.filter(intermediate => {
                return intermediate.Difficulty === 'Intermediate'
            })
            ...
            setIntermediateWorkout(exercise)
  ...
  return (
    <div style={{ marginRight: '40px', marginLeft: '40px', marginBottom: '40px'}}>
        ...
        <Box sx={{
            ...
            {intermediateWorkout.slice((page - 1) * itemsPerPage, page * itemsPerPage).map((workout, index) => (
                ...
                <Card key={index} sx={{ maxWidth: 345, height: '100%' }}>
                    <CardContent>
                        ...
                        {workout.targetMuscle.Primary && workout.targetMuscle.Primary.length > 0 && workout.targetMuscle.Primary.map((muscle, muscleIndex) => (
                            <Typography key={muscleIndex} variant="body2" component="div" sx={{fontSize: 15, marginTop: '5px', marginBottom: '5px'}}>
                                Target Muscle(s): {muscle}
                            </Typography>
                        ))}
            ...
            <Box sx={{
                ...
                <Pagination color="primary" count={Math.ceil(intermediateWorkout.length / itemsPerPage)} page={page} onChange={handlePageChange} />
            </Box>
