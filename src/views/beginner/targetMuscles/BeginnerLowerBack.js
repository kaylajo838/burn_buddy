...
export default function BeginnerLowerBack() {
    ...
    const fitnessTest = async () => {
        ...
        try {
            ...
            let workoutJson = result.filter(beginner => {
                ...
                return beginner.Difficulty === 'Beginner' && beginner.target && beginner.target.Primary && beginner.target.Primary.includes('Lower back')
            })
  ...
  return (
    <div style={{ marginRight: '40px', marginLeft: '40px', marginBottom: '40px'}}>
        <Button sx={{margin: '20px'}} onClick={() => navigate('/beginnerTarget')}>Back</Button>
        <Box sx={{
            display: 'flex',
