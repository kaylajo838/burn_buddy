...
export default function AdvancedObliques() {
    ...
    const fitnessTest = async () => {
        ...
        try {
            ...
            let workoutJson = result.filter(advanced => {
                ...
                return advanced.Difficulty === 'Advanced' && advanced.target && advanced.target.Primary && advanced.target.Primary.includes('Obliques')
            })
  ...
  return (
    <div style={{ marginRight: '40px', marginLeft: '40px', marginBottom: '40px'}}>
        <Button sx={{margin: '20px'}} onClick={() => navigate('/advancedTarget')}>Back</Button>
        <Box sx={{
            display: 'flex',
